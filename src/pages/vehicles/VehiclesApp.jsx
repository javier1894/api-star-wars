            ...
            {vehicles && <>
                {(vehicles?.next !== null || vehicles?.previous !== null) &&
                    <div className="flex flex-row justify-center items-center pb-5 gap-x-6">
                        {vehicles?.previous && <button onClick={() => { getNewPage(vehicles?.previous, 0) }}><ArrowLeftIcon /></button>}
                        <p className="font-semibold">Page {numPage}</p>
                        {vehicles?.next && <button onClick={() => { getNewPage(vehicles?.next, 1) }}><ArrowRightIcon /></button>}
                    </div>}
